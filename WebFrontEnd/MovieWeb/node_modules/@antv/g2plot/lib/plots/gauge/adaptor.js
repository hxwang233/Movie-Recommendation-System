...
function geometry(params) {
    ...
    if (indicator) {
        ...
        v1.point()
            .position(constants_1.PERCENT + "*1")
        ...
        // 一部分应用到 scale 中
        v1.scale(constants_1.PERCENT, utils_1.pick(axis, constant_1.AXIS_META_CONFIG_KEYS));
    }
    // 辅助 range
    // [{ range: 1, type: '0' }]
    ...
    v2.data(rangeData);
    var rangeColor = util_1.isString(color) ? [color, constants_1.DEFAULT_COLOR] : color;
    v2.interval().position("1*" + constants_1.RANGE_VALUE).color(constants_1.RANGE_TYPE, rangeColor).adjust('stack');
    v2.coordinate('polar', {
...
function meterView(params) {
    ...
    if (type === 'meter') {
        ...
        var v3 = chart.createView({ id: constants_1.MASK_VIEW_ID });
        v3.data([(_a = {}, _a[constants_1.RANGE_TYPE] = '1', _a[constants_1.RANGE_VALUE] = 1, _a)]);
        v3.interval()
            .position("1*" + constants_1.RANGE_VALUE)
            .color(color)
            .adjust('stack')
            .shape('meter-gauge')
            .customInfo(meter || {});
...
function adaptor(params) {
    ...
    common_1.animation, geometry, meta, statistic, common_1.interaction, common_1.theme, 
    // meterView 需要放到主题之后
    meterView, common_1.annotation(), other
    // ... 其他的 adaptor flow
    )(params);
}
